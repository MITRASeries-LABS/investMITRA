-- =============================================================================
-- investMITRA — Neon Schema (single transaction)
...
SET search_path TO investmitra, public;
...
CREATE INDEX IF NOT EXISTS idx_ca_manual ON corporate_actions (requires_manual_review) WHERE requires_manual_review = TRUE;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_own_isin_period ON ownership_data (isin, period_end, source_id);
...
CREATE TABLE IF NOT EXISTS macroeconomic_indicators (
    id               UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    indicator_id     VARCHAR(60) NOT NULL,
    source_id        VARCHAR(60) REFERENCES source_registry(source_id),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_macro_vintage ON macroeconomic_indicators (indicator_id, observation_date, data_vintage);
CREATE INDEX        IF NOT EXISTS idx_macro_obs     ON macroeconomic_indicators (indicator_id, observation_date DESC);
...
CREATE INDEX IF NOT EXISTS idx_prl_failed ON pipeline_run_log (status) WHERE status IN ('failed', 'quarantined');

-- EQUITY PRICES (partitioned by year)
...
CREATE TABLE IF NOT EXISTS equity_prices_2014 PARTITION OF equity_prices FOR VALUES FROM ('2014-01-01') TO ('2015-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2015 PARTITION OF equity_prices FOR VALUES FROM ('2015-01-01') TO ('2016-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2016 PARTITION OF equity_prices FOR VALUES FROM ('2016-01-01') TO ('2017-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2017 PARTITION OF equity_prices FOR VALUES FROM ('2017-01-01') TO ('2018-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2018 PARTITION OF equity_prices FOR VALUES FROM ('2018-01-01') TO ('2019-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2019 PARTITION OF equity_prices FOR VALUES FROM ('2019-01-01') TO ('2020-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2020 PARTITION OF equity_prices FOR VALUES FROM ('2020-01-01') TO ('2021-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2021 PARTITION OF equity_prices FOR VALUES FROM ('2021-01-01') TO ('2022-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2022 PARTITION OF equity_prices FOR VALUES FROM ('2022-01-01') TO ('2023-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2023 PARTITION OF equity_prices FOR VALUES FROM ('2023-01-01') TO ('2024-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2024 PARTITION OF equity_prices FOR VALUES FROM ('2024-01-01') TO ('2025-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2025 PARTITION OF equity_prices FOR VALUES FROM ('2025-01-01') TO ('2026-01-01');
CREATE TABLE IF NOT EXISTS equity_prices_2026 PARTITION OF equity_prices FOR VALUES FROM ('2026-01-01') TO ('2027-01-01');

CREATE INDEX IF NOT EXISTS idx_ep_isin_date ON equity_prices (isin, trade_date DESC);
CREATE INDEX IF NOT EXISTS idx_ep_quality   ON equity_prices (quality_score) WHERE quality_score < 70;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_fin_isin_period ON company_financials (isin, period_end, period_type, source_id);
CREATE INDEX        IF NOT EXISTS idx_fin_filing      ON company_financials (filing_date DESC);

-- AUTO-UPDATE updated_at
CREATE OR REPLACE FUNCTION investmitra.set_updated_at()
...
CREATE TRIGGER trg_cm_updated_at
    ...
    FOR EACH ROW EXECUTE FUNCTION investmitra.set_updated_at();
